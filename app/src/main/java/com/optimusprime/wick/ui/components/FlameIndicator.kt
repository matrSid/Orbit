...
/**
 * The same wick mark as the app icon, reused as a living status light: it
...
fun FlameIndicator(pose: FlamePose, modifier: Modifier = Modifier, size: Dp = 28.dp) {
    ...
    when (pose) {
        FlamePose.LIT -> {
            outerColor = WickColors.brass
            innerColor = WickColors.bone
            breathe = flicker
        }
        FlamePose.RESTING -> {
            outerColor = WickColors.ashFaint
            innerColor = WickColors.ash
            breathe = 1f
        }
        FlamePose.AWAY -> {
            outerColor = WickColors.oxide
            innerColor = WickColors.oxideSoft
            breathe = 1f
        }
    }
